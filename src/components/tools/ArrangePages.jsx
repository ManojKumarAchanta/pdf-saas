          ...
          <div className="space-y-6">
            <div className="bg-white border border-gray-200 rounded-lg p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{pageOrder.length} {strings.arrange.fileInfo.pages}</p>
                </div>
              </div>
              <button
                onClick={handleReset}
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-gray-900">{strings.arrange.pageOrder.title}</h2>
                <ViewToggle viewMode={viewMode} onViewChange={setViewMode} />
              </div>

              {viewMode === 'grid' ? (
                <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3" role="list">
                  {pageOrder.map((pageNum, index) => (
                    <div
                      ...
                      role="listitem"
                    >
                      <div className="text-xs font-medium text-gray-500 mb-1">#{index + 1}</div>
                      <div className="w-full aspect-[3/4] bg-gradient-to-br from-blue-50 to-blue-100 border-2 border-blue-200 rounded-md flex items-center justify-center mb-2 shadow-sm">
                        <div className="text-sm font-bold text-blue-700">{pageNum}</div>
                      </div>
                      <div className="text-xs text-gray-500">{strings.arrange.pageOrder.pageLabel} {pageNum}</div>
                    </div>
                  ))}
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className="flex gap-1">
                        <button
                          ...
                          type="button"
                        >
                          <div className="w-4 h-4" aria-hidden="true">{icons.arrowUp}</div>
                        </button>
                        <button
                          ...
                          type="button"
                        >
                          <div className="w-4 h-4" aria-hidden="true">{icons.arrowDown}</div>
                        </button>
                      </div>
                    </div>
                  ))}
